...
import com.sivert.schat2.ui.screens.base.BaseFragment
...
class SettingsFragment : BaseFragment(R.layout.fragment_settings) {
    ...
    private fun initFields() {
        ...
        setting_full_name.text = USER.fullname
        setting_phone_number.text = USER.phone
        setting_status.text = USER.status
        setting_username.text = USER.username
        setting_btn_change_username.setOnClickListener { replaceFragment(ChangeUsernameFragment()) }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE && resultCode == RESULT_OK && data !=null) {
            ...
            val path = REF_STORAGE_ROOT.child(
                ...
                .child(CURRENT_UID)
            putFileToStroge(uri, path) {
                getUrlFromStroge(path) {
                    putUrlToDatabase(it) {
